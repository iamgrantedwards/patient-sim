...
async def save_call(ctx: JobContext) -> None:
    ...
    try:
        ...
        artifacts.meta["reported_model_usage"] = [
            entry.model_dump(mode="json") for entry in report.model_usage or []
        ]
        ...
        try:
            ...
            # Evidence failures must never prevent closing the connected SIP leg.
            await ctx.delete_room()


async def shutdown_call(ctx: JobContext, reason: str) -> None:
    try:
        await ctx.delete_room()
    finally:
        # A failing room API must not prevent local worker shutdown.
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    dirty = bool(
        subprocess.check_output(["git", "status", "--porcelain"], cwd=PROJECT_ROOT, text=True)
    )
    artifacts = CallArtifacts(
        PROJECT_ROOT / "calls" / call_id,
        {
            "call_id": call_id,
    ...
    try:
        ...
        detector = (
            ...
            if cfg.turn_detection == "default"
            else cfg.turn_detection
        )
        session = AgentSession(
            ...
            turn_handling=TurnHandlingOptions(
                ...
                endpointing={
                    ...
                    "min_delay": cfg.endpointing_min_delay,
        ...
        await asyncio.wait_for(
            session.start(
                Agent(instructions=instructions, tools=list(end_tool.tools)),
                room=ctx.room,
                room_options=room_io.RoomOptions(
                    participant_identity="assessment-line",
                    text_input=False,
                    audio_input=room_io.AudioInputOptions(
                        noise_cancellation=None, auto_gain_control=False
                    ),
        ...
        request = api.CreateSIPParticipantRequest(
            ...
            sip_call_to=cfg.target_number,
            sip_number=cfg.caller_id,
            room_name=ctx.room.name,
            participant_identity="assessment-line",
            participant_name="Assessment line",
        ...
        try:
            info = await ctx.api.sip.create_sip_participant(
                request, timeout=cfg.ringing_timeout_seconds + 15
            )
        ...
        artifacts.meta["sip"].update(
            status="answered", answered_at=time.time(), call_id=info.sip_call_id
        )
        ...
        async def deadline():
            ...
            artifacts.end("failsafe")
            await shutdown_call(ctx, "failsafe")
        ...
        await shutdown_call(ctx, "worker_error")
