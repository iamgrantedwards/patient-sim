...
from .config import PROJECT_ROOT, PERMITTED_TARGET, load
...
async def dispatch(scenario: str) -> None:
    ...
    call_id = datetime.now(timezone.utc).strftime('call-%Y%m%d-%H%M%S-') + uuid.uuid4().hex[:8]
    ...
    async with api.LiveKitAPI(cfg.livekit_url, cfg.livekit_api_key, cfg.livekit_api_secret) as client:
        try:
            created = await client.agent_dispatch.create_dispatch(api.CreateAgentDispatchRequest(
                agent_name='patient-sim', room=call_id,
            ...
            write_json(directory / 'dispatch.json', record)
            raise RuntimeError(f'Dispatch failed ({type(error).__name__}); inspect {directory}/dispatch.json. No automatic retry.') from None
...
def main() -> None:
    ...
    action.add_argument('--call', action='store_true', help='Place one real call; requires a running worker')
    action.add_argument('--dry-run', action='store_true', help='Show the patient prompt without network calls (default)')
