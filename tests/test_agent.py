...
def test_worker_prepares_audio_before_dialing_and_uses_server_limits(monkeypatch, tmp_path, configured):
    order = []
    sessions = []
    ...
    monkeypatch.setattr(agent.subprocess, 'check_output', lambda args, **kw: 'testrevision' if args[1] == 'rev-parse' else '')
    ...
    ctx = SimpleNamespace(
        job=SimpleNamespace(id='job-fixture', metadata=json.dumps({'call_id': 'call-fixture', 'scenario': 'smoke'})),
        room=Room(), connect=connect, api=SimpleNamespace(sip=SimpleNamespace(create_sip_participant=place)),
        add_shutdown_callback=callbacks.append,
        shutdown=lambda **kw: shutdown_reasons.append(kw['reason']), delete_room=AsyncMock(),
    )
    ...
    async def run():
        ...
        for index in range(configured.max_turns):
            ...
            event = SimpleNamespace(item=item, model_dump=lambda **kw: {'item': {'type': 'message'}})
