...
@pytest.mark.parametrize('code,expected', [(408, 'no_answer'), (480, 'no_answer'),
    (486, 'rejected'), (603, 'rejected'), (500, 'sip_error'), (None, 'sip_error')])
def test_sip_failures(code, expected):
    assert sip_failure(code) == expected


@pytest.mark.parametrize('reason,prior,expected', [
...
@pytest.mark.parametrize('reason,expected', [
    ('MEDIA_FAILURE', 'telephony_error'), ('CONNECTION_TIMEOUT', 'telephony_error'),
    ('SIP_TRUNK_FAILURE', 'sip_error'), ('USER_UNAVAILABLE', 'no_answer'),
    ('USER_REJECTED', 'rejected'), ('CLIENT_INITIATED', 'remote_hangup'),
