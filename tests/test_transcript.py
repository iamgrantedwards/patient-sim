...
def message(item_id, role, text, interrupted=False):
    return event('conversation_item_added', {'item': {
        'id': item_id, 'type': 'message', 'role': role, 'content': [text], 'interrupted': interrupted,
    }})


def test_interim_and_final_stt_do_not_duplicate_committed_turns():
    events = [event('user_input_transcribed', {'transcript': 'We close', 'is_final': False}),
...
def test_partial_output_and_uncommitted_input_survive_recovery(tmp_path):
    ...
    artifacts.append('conversation_item_added', message('one', 'assistant', 'I would like', True)['data'])
